IF EXISTS (SELECT TOP 1 1 FROM [sys].[databases] WHERE [name]='fabcon_source_basic')
BEGIN
    EXEC('DROP DATABASE [fabcon_source_basic]')
END
GO
...
SELECT @currenttime= MAX([CreatedOn]) FROM [dbo].[Transactions]
SET @daysdiff = DATEDIFF(DAY, @currenttime, DATEADD(DAY, -5, GETDATE()))
PRINT ''
PRINT '@currenttime: ' + FORMAT(@currenttime, 'yyyy-MM-dd HH:mm:ss.fffff')
PRINT '   @daysdiff: ' + FORMAT(@daysdiff, '0')
PRINT ''
IF @daysdiff > 0
    EXEC [dbo].[usp_insert_range_transaction] @days = @daysdiff
ELSE
    DELETE FROM [dbo].[Transactions] WHERE [CreatedOn] > DATEADD(DAY, -5, GETDATE())
GO

DECLARE @newtransaction DATETIME2 = (SELECT MAX([CreatedOn]) FROM [dbo].[Transactions])
PRINT 'New current CreatedOn ' + FORMAT(@newtransaction, 'yyyy-MM-dd HH:mm:ss.fffff')
